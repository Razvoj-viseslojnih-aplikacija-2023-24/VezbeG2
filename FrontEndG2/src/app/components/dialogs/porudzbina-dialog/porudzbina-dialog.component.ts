...
export class PorudzbinaDialogComponent implements OnInit {
  ...
  public add(){
    this.service.addPorudzbina(this.data).subscribe(
      (data) => {
        this.snackBar.open(`Uspesno dodata porudzbina sa ID: ${data.id}`, `U redu`, {duration:2500});
      }
    ),
    (error:Error) => {
  ...
  public update(){
    this.service.updatePorudzbina(this.data).subscribe(
      (data) => {
        this.snackBar.open(`Uspesno azurirana porudzbina sa ID: ${data.id}`, `U redu`, {duration:2500});
      }
    ),
    (error:Error) => {
      console.log(error.name + ' ' + error.message);
