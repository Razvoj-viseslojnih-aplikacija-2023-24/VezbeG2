...
export class ArtiklDialogComponent {
  ...
  public add(){
    this.service.addArtikl(this.data).subscribe(
      (data) => {
        this.snackBar.open(`Uspesno dodat artikl sa nazivom: ${data.naziv}`, `U redu`, {duration:2500});
      }
    ),
    (error:Error) => {
  ...
  public update(){
    this.service.updateArtikl(this.data).subscribe(
      (data) => {
        this.snackBar.open(`Uspesno azuriran artikl sa nazivom: ${data.naziv}`, `U redu`, {duration:2500});
      }
    ),
    (error:Error) => {
      console.log(error.name + ' ' + error.message);
